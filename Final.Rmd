...
Since the p value of `r p_value_h2s` is less than the significance level of 0.05, the null hypothesis can be rejected as the test is statistically significant.  
...
Since the p value of `r p_value_lactic` is less than the significance level of 0.05, the null hypothesis can be rejected as the test is statistically significant.  
...
The model with only Acetic as a predictor is stronger. I prefer that model over the one with both Acetic and H2S as predictors because it is more significant, with a P-value of 0.001658 compared to 0.4065277 for the previous.  
...
beta <- modelCoeffs["h2s", "Estimate"]
beta2 <- modelCoeffs["lactic", "Estimate"] 

se <- modelCoeffs["h2s", "Std. Error"]
se2 <- modelCoeffs["lactic", "Std. Error"]
...
t_value2 <- beta2/se2
p_value2 <- 2*pt(-abs(t_value2), df=nrow(loss) - 4)

p_value_total <- pf(modelSummary$fstatistic[1],modelSummary$fstatistic[2],modelSummary$fstatistic[3],lower.tail=FALSE)
```
...
F-statistic P-value: `r p_value_total`  
The results obtained from using both predictors in a model were much better than the results obtained from the analysis of a simple linear regression of the explanatory variables on their own. As seen above, the p-value obtained when H2S and lactic are both used as predictors is `r p_value_total` When they used as individual explanatory variables, their p-values are `r p_value_h2s` and `r p_value_lactic` for H2S and lactic, respectively. When they are both used as predictors together, the p-value was decreased which shows that the test is more significant and thus more reliable as a predictor for the taste of the cheese.
