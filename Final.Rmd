...
Using both H2S and Lactic as predictors in the model resulted in a better model than that of each of the explanatory variables on their own. As seen above, the p-value obtained when H2S and lactic are both used as predictors is `r p_value_total` When they used as individual explanatory variables, their p-values are `r p_value_h2s` and `r p_value_lactic` for H2S and lactic, respectively. When they are both used as predictors together, the p-value is lower which shows that the model is more significant and thus more reliable as a predictor for the taste of the cheese.
...
The model has an R-squared value of `r modelSummary$r.squared`. There is a strong positive relationship between the explanatory variables and taste. The model has a P-value of `r p_value`. At the significance level of 0.05, we have statistically significant evidence to suggest that a positive relationship exists between the explanatory variables and taste.  
