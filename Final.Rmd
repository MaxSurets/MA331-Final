...
The results obtained from using both predictors in a model were much better than the results obtained from the analysis of a simple linear regression of the explanatory variables on their own. As seen directed above, the t-value for H2S is `r t_value` and lactic is `r t_value2` when they are both used as predictors. When they used as individual explanatory variables, their t-values are `r t_value_h2s` and `r t_value_lactic` for H2S and lactic, respectively. When they are both used as predictors together, the t-value for both variables was decreased. This decrease is important as the closer to 0 that the t-value is, the greater the evidence that the null hypothesis is not rejected which means that the predicted value would be a better estimate.
